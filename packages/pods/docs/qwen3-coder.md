...
[Qwen3-Coder](https://github.com/QwenLM/Qwen3-Coder)是阿里云Qwen团队创建的高级大语言模型。 vLLM 已支持 Qwen3-Coder，并且 `tool-call` 功能将在 vLLM v0.10.0 及更高版本中提供 您可以使用以下方法安装具有 `tool-call` 支持的 vLLM：
...
### 在 8xH200（或 H20）GPU 上运行（141GB × 8）
...
### 基准测试
我们使用以下脚本对 `Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8` 进行基准测试
...
### BF16 型号
- **上下文长度限制**：单个 H20 节点无法提供原始上下文长度 (262144)。您可以减少 `max-model-len` 或增加 `gpu-memory-utilization` 以在内存限制内工作。

### FP8 型号
- **上下文长度限制**：单个 H20 节点无法提供原始上下文长度 (262144)。您可以减少 `max-model-len` 或增加 `gpu-memory-utilization` 以在内存限制内工作。
- **DeepGEMM 用法**：要使用 [DeepGEMM](https://github.com/deepseek-ai/DeepGEMM)，请设置 `VLLM_USE_DEEP_GEMM=1`。按照 [setup instructions](https://github.com/vllm-project/vllm/blob/main/benchmarks/kernels/deepgemm/README.md#setup) 进行安装。
- **张量并行问题**：使用 `tensor-parallel-size 8` 时，预计会出现以下故障。使用 `--data-parallel-size` 切换到数据并行模式。
- **其他资源**：请参阅 [Data Parallel Deployment documentation](https://docs.vllm.ai/en/latest/serving/data_parallel_deployment.html) 了解更多并行组。
